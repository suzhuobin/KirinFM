...
public class FmFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    private void initImag() {
        Banner banner = find(R.id.banner);
        ...
        banner.setImages(imagUrls).setBannerStyle(BannerConfig.CIRCLE_INDICATOR_TITLE_INSIDE).setBannerTitles(titles)
                .setBannerAnimation(Transformer.Tablet).setImageLoader(new GlideImageLoader()).setOnBannerListener(new OnBannerListener() {
            @Override
            public void OnBannerClick(int position) {
                ViewUtils.showPlayBill(getContext(), "返回电台",
                        titles.get(position), new ArrayList<>());
            }
        }).start();
    /*
        banner.setImages(imagUrls);
        //设置banner动画效果
